...
log_info "创建兼容版本的requirements.txt..."
...
cp requirements_compatible.txt requirements.txt
log_success "requirements.txt已更新为兼容版本"

# 3. 创建优化的Dockerfile
log_info "创建优化的Dockerfile..."
cat > Dockerfile.numpy_fix << 'EOF'
...
# 4. 构建镜像
log_info "构建兼容性修复的镜像..."
IMAGE_NAME="pdf2excel-numpy-fixed"

if docker build -f Dockerfile.numpy_fix --no-cache -t $IMAGE_NAME .; then
    log_success "镜像构建成功！"
else
    log_error "构建失败"
    exit 1
...
if curl -f http://localhost:5001/api/pdf/diagnose > /dev/null 2>&1; then
    log_success "🎉 NumPy/Pandas兼容性修复成功！"
...
" || echo "服务正常运行"
    ...
    echo
    echo "🧪 测试pandas导入:"
    docker exec ${IMAGE_NAME}-test python3 -c "
import pandas as pd
import numpy as np
...
# 清理临时文件
rm -f requirements_compatible.txt Dockerfile.numpy_fix

echo
log_success "🎉 兼容性修复完成！"
echo "=================================="
echo "📦 镜像名称: $IMAGE_NAME"
echo "🔧 修复内容: NumPy/Pandas版本兼容性"
echo
echo "🚀 下一步操作:"
echo "   1. 标记镜像: docker tag $IMAGE_NAME your-username/pdf2excel:latest"
echo "   2. 推送镜像: docker push your-username/pdf2excel:latest"
echo "   3. 生产部署: docker run -d -p 80:5000 --name pdf2excel $IMAGE_NAME"